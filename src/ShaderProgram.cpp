...
ShaderProgram::ShaderProgram(std::string vert, std::string frag)
{
  ...
  {
	  GLint maxLenght = 0;
	  glGetShaderiv(vertexShaderId, GL_INFO_LOG_LENGTH, &maxLenght);
	  std::vector<GLchar> errorLog(maxLenght);
	  glGetShaderInfoLog(vertexShaderId, maxLenght, &maxLenght, &errorLog[0]);
  ...
  {
	  GLint maxLenght = 0;
	  glGetShaderiv(fragmentShaderId, GL_INFO_LOG_LENGTH, &maxLenght);
	  std::vector<GLchar> errorLog(maxLenght);
	  glGetShaderInfoLog(fragmentShaderId, maxLenght, &maxLenght, &errorLog[0]);
	  std::cout << &errorLog.at(0) << std::endl;
    throw std::exception();
  }
